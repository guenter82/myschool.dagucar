...
public class TcpServer implements Runnable {
	...
	public TcpServer() {
		...
		this.ip = ServerContext.context.ip;
	}
	...
	public void run() {
		...
		InputStream is=null;
		try {
			InetAddress adr = InetAddress.getByName(this.ip);
			server = new ServerSocket(this.port, 50, adr);
			...
			for (int i=0; i<10000; i++) {
				...
				if (first != ServerContext.firstHandshakeByte) {
					continue;
				}
				byte second=(byte) is.read();
				if (second != ServerContext.secondHandshakeByte) {
					continue;
				}
				...
				if (serviceByte == ServerContext.handleCommandServiceByte) {
					...
					if (daguCar == null) {
						...
						log.error("DaguCar with number " + daguCar + " could not be found! Command will be ignored.");
					}
					...
					if (code != -1) {
						...
						if ( (nextByte = is.read()) != -1 ) {
							millisecondsBytes[0] = (byte)nextByte;
						} else {
							throw new IllegalStateException("Could not read reapting time because stream is closed.");
						}
						if ( (nextByte = is.read()) != -1 ) {
							millisecondsBytes[1] = (byte)nextByte;
						} else {
							throw new IllegalStateException("Could not read reapting time because stream is closed.");
						}
						if ( (nextByte = is.read()) != -1 ) {
							millisecondsBytes[2] = (byte)nextByte;
						} else {
							throw new IllegalStateException("Could not read reapting time because stream is closed.");
						}
						if ( (nextByte = is.read()) != -1 ) {
							millisecondsBytes[3] = (byte)nextByte;
						} else {
							throw new IllegalStateException("Could not read reapting time because stream is closed.");
						}
